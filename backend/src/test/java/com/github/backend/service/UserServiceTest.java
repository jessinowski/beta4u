...
//        OAuth2User oAuth2User = mock(OAuth2User.class);
//        Map<String, Object> attributes = new HashMap<>();
//        attributes.put("id", "22");
//        attributes.put("avatar_url", "image");
//        when(oAuth2User.getAttributes()).thenReturn(attributes);
//        User user =new User(
//                "22",
//                "jurassica",
...
//                List.of());
//
//        when(repo.findById(id)).thenReturn(Optional.of(boulder));
...
//    }
    ...
    void getMyFavorites() {
        ...
        UserDto user =new UserDto(
                ...
                List.of(Hold.CRIMP),
                List.of(Style.MANTLE));
        User currentUser = service.createUser(oAuth2User, user);
        when(repo.findById(currentUser.getId()).orElseThrow().getMyFavorites()).thenReturn(expected);
        //WHEN
        List<Boulder> actual = service.getMyFavorites(oAuth2User);
        //THEN
        assertEquals(expected, actual);
        verify(repo).findById("22");
    }
